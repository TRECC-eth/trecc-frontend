...
import { TrendingUp, DollarSign, Percent, Layers, Bot } from 'lucide-react';
      ...
      <MetallicCard accent="silver" contentClassName="p-4" className="col-span-12 sm:col-span-6 lg:col-span-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-white text-lg font-medium tabular-nums tracking-tight">${MOCK_LENDER.netProfit.toLocaleString()}</p>
            <p className="text-emerald-400 text-xs font-medium">+{MOCK_LENDER.netProfitPercent}%</p>
          </div>
        </div>
      </MetallicCard>
      <MetallicCard accent="silver" contentClassName="p-4" className="col-span-12 sm:col-span-6 lg:col-span-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-white text-lg font-medium tabular-nums tracking-tight">{MOCK_LENDER.apy}%</p>
          </div>
        </div>
      </MetallicCard>
      <MetallicCard accent="silver" contentClassName="p-4" className="col-span-12 sm:col-span-6 lg:col-span-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-white text-lg font-medium tabular-nums tracking-tight">${MOCK_LENDER.portfolioValue.toLocaleString()}</p>
          </div>
        </div>
      </MetallicCard>
      ...
      <div className="col-span-12 lg:col-span-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4 min-h-0 auto-rows-fr">
          ...
          <ul className="space-y-2 overflow-auto min-h-0 flex-1 pr-1">
            {MOCK_LENDER.protocols.map((p) => (
              <li key={p.name} className="flex justify-between items-center py-2 px-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors text-xs">
                ...
                <span className="text-zinc-500 tabular-nums">{p.share}% · ${p.tvl.toLocaleString()}</span>
              </li>
            ))}
          </ul>
        </MetallicCard>
        <MetallicCard accent="slate" contentClassName="p-5 flex flex-col min-h-0" className="min-h-0 flex flex-col">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-medium text-white tracking-wide">Agents Borrowed</h3>
          </div>
          <ul className="space-y-2 overflow-auto min-h-0 flex-1 pr-1">
