...
function usePortfolioData(initialValue: number) {
  const [data, setData] = useState<{ time: number; value: number }[]>(() => {
    ...
    let v = initialValue * 0.94;
    for (let i = 24 * 12; i >= 0; i--) {
      const t = now - i * 300;
      v = v + (Math.random() - 0.46) * initialValue * 0.002;
      points.push({ time: t, value: Math.max(initialValue * 0.82, v) });
    }
    return points;
  });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const now = Math.floor(Date.now() / 1000);
      const drift = (Math.random() - 0.46) * initialValue * 0.0018;
      setValue((prev) => {
        const next = Math.max(initialValue * 0.85, prev + drift);
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="relative z-10 p-4 md:p-8 max-w-6xl mx-auto">
        ...
        <header className="mb-10">
          ...
          <p className="text-neutral-500 mt-1">Agent performance and protocol exposure</p>
        </header>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Agents', value: String(MOCK_AGENTS.length), icon: Bot },
            ...
            <ul className="space-y-3">
              {MOCK_AGENTS.map((agent) => (
                ...
                >
                  ...
                  <span className="text-neutral-300 text-sm font-medium">{agent.status}</span>
                </li>
              ))}
            </ul>
          </section>
